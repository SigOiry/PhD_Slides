...
speclib <- "Data/Part1/spec_lib.csv" %>% 
    ...
    mutate(Class = case_when(Class == "Magnoliosida" ~ "Magnoliopsida",
                             ...
                             Class == "Rhodophyceae" ~ "Florideophyceae",
                             TRUE ~ Class)) %>% 
    group_by(Class) %>% 
    reframe(Wavelength = Wavelength,
            Ref = (Ref-min(Ref))/(max(Ref)-min(Ref))) %>% 
  mutate(Ref = case_when(Class == "Chlorophyceae" ~ log_func(Wavelength)*Ref,
                         T ~ Ref))
    

colscale <- c("Bacillariophyceae" = "#DAA520", 
              "Chlorophyceae" = "#b3ff1a", 
              "Magnoliopsida" = "#389318", 
              "Phaeophyceae" = "#873e23", 
              "Florideophyceae" = "#b3002d")
...
::: {.absolute top="10%" right="0%" .fragment .fade-in-then-out fragment-index="3"}
![](Images/Part1/Figs_vegetations.png){height="1250"}
:::
...
::: {.absolute top="10%" left="0%" .fragment .fade-in fragment-index="5"}
